 ...
 */
#ifdef HC_LEGACY_PAR
#  define _ (Hlong*)0   /* anonymous variable             */
#  define _i (Hlong*)0  /* anonymous variable (long)      */
#  define _d (double*)0 /* anonymous variable (double)    */
#  define _s (char*)0   /* anonymous variable (string)    */
#  define _t (Htuple*)0 /* anonymous variable (Tupel)     */
#endif

#define HALCONC_HNULL ((Hphandle)0) /* undefined handle         */

#define NO_OBJECTS (Hobject)(1)   /* empty list of objects      */
#define EMPTY_REGION (Hobject)(2) /* object with empty region   */
...
LIntExport int  IsHcInterfaceStringEncodingUtf8(void);
...
/* Obsolete method, kept inside for compatibility reasons, always returns 0
 *---------------------------------------------------------------------------*/
...
/*
 * The following functions are used to support dictionaries in HDevelop export
 *---------------------------------------------------------------------------*/
LIntExport Herror T_test_equal_dict_item(const Htuple dicts, const Htuple keys,
                                         Htuple* result);

/*
 * The following functions are kept for backwards compatibility only.
 * DO NOT USE !!!
...
#else /* := #ifndef HC_FAST -> use versions with error checking */
