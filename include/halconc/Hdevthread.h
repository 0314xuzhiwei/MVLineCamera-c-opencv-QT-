...
LIntExport Herror clear_hdevthread_context(Hdevthread_context context_handle);
 ...
 * thread. */
LIntExport Herror create_hdevthread(Hdevthread_context const context_handle,
                                    void const* proc, Hlong ip_num,
                                    Hlong                  op_num,
                                    Hdevthread* H_RESTRICT hthread_handle);

/* exit thread is called at the end of the thread's execution and clears
 * the hthread handle. */
LIntExport Herror exit_hdevthread(Hdevthread hthread_handle);
 ...
 * explicitly. */
LIntExport Herror clear_hdevthread(Hdevthread hthread_handle);
...
LIntExport Herror set_hdevthread_iop(Hdevthread hthread_handle, Hlong parpos,
                                     Hobject const obj);
 ...
 * starting with 0.*/
LIntExport Herror bind_hdevthread_oop(Hdevthread hthread_handle, Hlong parpos,
                                      bool glb_flag, Hobject* obj);
...
LIntExport Herror set_hdevthread_ivp(Hdevthread hthread_handle, Hlong parpos,
 ...
 * parameter within the signature of proc, starting with 0.*/
LIntExport Herror bind_hdevthread_ovp(Hdevthread hthread_handle, Hlong parpos,
                                      bool glb_flag, Hvector* vector,
...
LIntExport Herror set_hdevthread_icp(Hdevthread hthread_handle, Hlong parpos,
                                     Htuple const tpl);
 ...
 * starting with 0.*/
LIntExport Herror bind_hdevthread_ocp(Hdevthread hthread_handle, Hlong parpos,
                                      bool glb_flag, Htuple* H_RESTRICT tpl);
 ...
 * set_hdevthread_iop. */
LIntExport Herror get_hdevthread_iop(Hdevthread const hthread_handle,
                                     Hlong parpos, Hobject* H_RESTRICT* obj);

/* overwrite_hdevthread_oop writes back the output object parameter on the
 * reference passed in set_hdevthread_oop. */
LIntExport Herror store_hdevthread_oop(Hdevthread hthread_handle, Hlong parpos,
                                       Hobject const obj);

/* get_hdevthread_ivp returns a reference on the copied vector parameter
 * set with set_hdevthread_ivp. */
LIntExport Herror get_hdevthread_ivp(Hdevthread const hthread_handle,
 ...
 * set_hdevthread_ovp. */
LIntExport Herror store_hdevthread_ovp(Hdevthread hthread_handle, Hlong parpos,
                                       Hvector const vector);

/* get_hdevthread_icp returns a reference on the copied tuple control parameter
 * set with set_hdevthread_icp. */
LIntExport Herror get_hdevthread_icp(Hdevthread const hthread_handle,
...
LIntExport Herror store_hdevthread_ocp(Hdevthread hthread_handle, Hlong parpos,
                                       Htuple const tpl);

/* start_hdevthread is called by the father thread to start the thread after
 * the parameters are set. Contrary to par_start_hdevthread, start_hdevthread
 * can be used when no thread has to join with the started thread. No thread id
...
LIntExport Herror start_hdevthread(Hdevthread const hthread_handle);

/* par_start_hdevthread is called by the father thread to start the thread
 * after the parameters are set. Contrary to start_thread, a thread id
 * (par_handle) is returned that can be used by par_join to wait for the
 * started thread exiting. Attention: Different to HDevelop, the child thread
 * must not run anymore when exiting the program or unloading the HALCON
 * library. Violating this rule might lead to a segmentation fault on
 * exit/unload, especially on unix systems. */
LIntExport Herror par_start_hdevthread(Hdevthread const   hthread_handle,
...
LIntExport Herror call_hdevthread_proc(Hdevthread const hthread_handle);

/* T_par_join_hdevthread waits until all threads addressed by their thread id
 * in par_handle finished. */
LIntExport Herror T_par_join_hdevthread(Htuple const par_handles);
