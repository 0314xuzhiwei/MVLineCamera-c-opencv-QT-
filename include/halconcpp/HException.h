...
public:
  ...
  static void Throw(const char* proc_name, const char* msg,
                    Herror err = H_MSG_OK);
  ...
  static void GetExceptionData(const HTuple& exception, const HTuple& name,
                               HTuple* value);
  ...
  const HString& ProcName() const
  {
    return mProcName;
  }
  const HString& ErrorMessage() const
  {
    return mErrorMessage;
  }
  Herror ErrorCode() const
  {
    return mErrorCode;
  }
...
private:
  void InitException(const HString& proc_name, const HString& msg, Herror err,
                     const HTuple& user_data);
};
...
{
public:
  HVectorAccessException(void) : HException("", "Illegal operation on vector")
  {
  }
