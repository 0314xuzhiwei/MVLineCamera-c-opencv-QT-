...
#  define HOI_COMPUTE_DEVICES                                                 \
    (1 << 0) /* Bit0: information for compute device                          \
              * support available */
#else
#  define HOI_COMPUTE_DEVICES 0
#endif

#define H_OPERATOR_INFO_DERIVATE                                              \
  ((H_OPERATOR_INFO_VERSION << 16) | HOI_COMPUTE_DEVICES)
 ...
 * compatibility.
 *---------------------------------------------------------------------------*/
extern HLibExport Herror HXPkgOpenId(int32_t xpkg_id, uint32_t halcon_version,
                                     uint32_t opinfo_version, int op_num,
                                     int* osid);
 ...
 * operator.
 *---------------------------------------------------------------------------*/
extern HLibExport Herror HXPkgSetOpInfo(int32_t xpkg_id, int32_t op_id,
                                        int32_t oi_id, ...);
...
typedef struct
{
  ...
  int8_t freeIOP[MAX_INP_OBJ_PAR + 1];
  ...
  Hctuple*
          InpCtrlStack[MAX_INP_CTRL_PAR]; /* stores interf./Single references*/
  Hctuple InpCtrlNested[MAX_INP_CTRL_PAR]; /* holds tuple trunks, eg for pure
                                            * type input ctrls(single values)*/
  bool ctrl_inherited; /* true, if all ctrl. values are inherited from
  ...
  Hctuple  OutpCtrlNested[MAX_OUTP_CTRL_PAR]; /* holds tuple trunks for output
                                               ...
                                               * operators)   */
  uint32_t
      freeOCP; /* bitcoded free flag: 8*sizeof(freeOCP)>=MAX_OUTP_CTRL_PAR*/
...
typedef struct HALCON_OPERATOR_HANDLE
{
  ...
  void*     generator;
  int32_t   proc_index;         /* Index of HALCON operator        */
