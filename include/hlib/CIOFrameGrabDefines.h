...
typedef struct _FGInstance
{
  ...
  bool    external_trigger;          /* trigger mode                         */
  ...
  bool    async_grab;            /* true <=> async grabbing engaged      */
  Himage* image;                 /* image objects to grab into (aux.)    */
  void*   gen_pointer;           /* generic pointer (auxiliary)          */
  /* -------------------- external memory allocation ---------------------- */
  bool halcon_malloc;        /* true <-> standard memory allocation  */
                             /* by HNewImage                         */
...
typedef struct _FGClass
{
  ...
  bool        available;     /* supported for the current platform    */
  ...
  Herror (*GrabData)(Hproc_handle proc_id, FGInstance* fginst, Himage** image,
                     int** num_channel, int* num_image, Hrlregion*** region,
                     int* num_region, Hcont*** cont, int* num_cont,
                     Hcpar** data, int* num_data);
  Herror (*GrabDataAsync)(Hproc_handle proc_id, FGInstance* fginst,
                          double maxDelay, Himage** image, int** num_channel,
                          int* num_image, Hrlregion*** region, int* num_region,
                          Hcont*** cont, int* num_cont, Hcpar** data,
                          int* num_data);
  Herror (*Info)(Hproc_handle proc_id, int queryType, char** info,
  ...
  Herror (*SetLut)(Hproc_handle proc_id, FGInstance* fginst, INT4_8* red,
                   INT4_8* green, INT4_8* blue, int num);
 ...
 * the symbol FG_FLAGS_NAME, which must be an int32_t. Note that if the
 * interface is openend using the HAddFgClass function, the flags cannot be
 * set.
 *
