...
#define DB_ID(PTR) ((int32_t)(((DBImage*)PTR)->magic) - DB_MAGIC_OK)
#define DB_ERROR_ID(PTR) ((int32_t)(((DBImage*)PTR)->magic) - DB_MAGIC_CLEARED)
...
#  define HPAR_DATASIZE_IMG(DOMAIN, CHANNELS, DATASIZE)                       \
    ...
    {                                                                         \
      (DATASIZE) = (HINT4_8)HGetFVal(DOMAIN, area) * (CHANNELS);              \
    }                                                                         \
    else if (0 < (DOMAIN)->num)                                               \
    {                                                                         \
      (DATASIZE) = (CHANNELS) * (HINT4_8)(DOMAIN)->num *                      \
...
typedef struct DB_REGION
{
  ...
  uint16_t          creator; /* operator index (see HProcInfo)         */
  ...
  bool              filed; /* is object stored in data base?         */
} DBRegion;
...
/* HCD_PTR::type values */
#  define HCD_BUFFER (int16_t)0
#  define HCD_IMAGE (int16_t)1
...
#define HCD_HOST_MEM_ALLOCATED (uint32_t)(1 << 0)
#define HCD_DEVICE_MEM_ALLOCATED (uint32_t)(1 << 1)
#define HCD_IMAGE_IN_HOST_MEM (uint32_t)(1 << 2)
#define HCD_IMAGE_IN_DEVICE_MEM (uint32_t)(1 << 3)
#define HCD_IMAGE_IN_IMAGE_2D (uint32_t)(1 << 4)
#define HCD_HOST_IS_PINNED (uint32_t)(1 << 5)
#define HCD_HAS_COMPUTE_EVENT (uint32_t)(1 << 6)
#define HCD_HAS_UPLOAD_EVENT (uint32_t)(1 << 7)
...
{
  uint32_t    magic;         /* specific magic number for tests        */
  ...
  uint16_t          num;     /* num. of gray value matrices; not fixed!*/
  uint16_t          creator; /* operator index (see HProcInfo)         */
...
/*                             External Interface                            */
/*===========================================================================*/
extern HLibExport Herror HDataBaseSwitch(bool db_on);
extern HLibExport Herror HDataBaseState(bool* db_on);
extern HLibExport Herror HDBIsInitial(bool* is_initial);
...
extern HLibExport Herror DBInitHandle(Hproc_handle proc_handle, bool shared);
